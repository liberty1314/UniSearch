...
UniSearch是一个高性能的网盘资源搜索API服务，支持TG搜索和自定义插件搜索。系统设计以性能和可扩展性为核心，支持并发搜索、结果智能排序和网盘类型分类。
...
在 Github 上先[![Fork me on GitHub](https://raw.githubusercontent.com/fishforks/fish2018/refs/heads/main/forkme.png)](https://github.com/fish2018/UniSearch/fork)
本项目，并点上 Star !!!
...
```
docker run -d --name UniSearch -p 80:80 ghcr.io/fish2018/UniSearch-web
```

##### 使用Docker Compose（推荐）
```
# 下载配置文件
curl -o docker-compose.yml https://raw.githubusercontent.com/fish2018/UniSearch-web/refs/heads/main/docker-compose.yml
...
```bash
docker run -d --name UniSearch -p 8888:8888 -v UniSearch-cache:/app/cache -e CHANNELS="tgsearchers3,xxx" ghcr.io/fish2018/UniSearch:latest
```
...
```bash
# 下载配置文件
curl -o docker-compose.yml  https://raw.githubusercontent.com/fish2018/UniSearch/refs/heads/main/docker-compose.yml
...
```linux
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags="-s -w -extldflags '-static'" -o UniSearch .
```
...
command=/home/work/UniSearch/UniSearch
directory=/home/work/UniSearch
autostart=true
autorestart=true
startsecs=5
startretries=3
...
server {
    listen 80;
    server_name UniSearch.252035.xyz;
...
server {
    ...
    access_log /home/work/logs/UniSearch.log;
...
[![Star History Chart](https://api.star-history.com/svg?repos=fish2018/UniSearch&type=Date)](https://star-history.com/#fish2018/UniSearch&Date)
